...
class Conta():
    ...
    def setSenha(self, novaSenha):
        ...
        print("Mudanca de senha realizada com sucesso!")

    def saque(self, senha, valor, op):
        """
        ...
        if senha == self.__senha:
            if self._saldo >= valor:
                ...
                if op == 1:
                    print(
                        f"Saque no valor de R$ {valor} realizado com sucesso")
                elif op == 4:
                    print(
                        f"Transferência no valor de R$ {valor} realizada com sucesso")
            else:
    ...
    def exibeDadosBeneficiarioTransferencia(self):
        """
        Método para exibição das informações da conta
        para transferidor conferir se beneficiario eh esse mesmo
        """
        print("Dados do beneficiário da transferencia:")
        print("Número: ", self.numero)
        print("Titular: ", self.titular)

    def valida_senha_conta(self, senha):
        """
        Método para validação se senha informada pelo usuário condiz
        com a senha da conta de número previamente informado
        """
...
# Cria uma classe derivada da classe conta,mantendo todas as funções já definidas em Conta
class ContaFuncionario(Conta):
    """
    Classe Conta Funcionario
    """

    def __init__(self, numero, titular, senha, funcao):
        """
        Construtor da classe Conta
        :param numero: número da conta
        :param titular: nome o titular da conta
        ...
        """
        super().__init__(numero, titular, senha,
                         funcao)  # super() é utilizado para se referir a uma função da classe na qual essa é derivada, no caso o construtor da classe Conta
        self.funcao = funcao
